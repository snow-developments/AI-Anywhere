...
- **Pick one canonical name per concept, project-wide, and never deviate.** Concrete list for this repo:
  ...
  - A repository's backing context field is always `_db` (private, readonly).
  ...
  - The ACP subprocess wrapper is always `AgentProcess`/`_agentProcess` — never `_process`, `_agent`, or `_client` (the last is especially confusable with `ClientSideConnection`, which is `acp-csharp`'s type, not ours).
...
- `_camelCase` (leading underscore) for private instance fields. No leading underscore on anything else.
...
- Async methods are suffixed `Async` (`InsertAsync`, `SendPromptAsync`) — no exceptions, including for `void`-returning fire-and-forget methods (rare; prefer `Task`-returning even then).
...
- `var` when the right-hand side makes the type obvious (`var profile = new AgentProfile { ... }`, `var results = await _db.Profiles.ToListAsync()`); the explicit type when it doesn't (`int id = ExecuteScalar(...)` if `ExecuteScalar`'s return type isn't visible at a glance).

## Prefer one-liners and bare blocks for simple, singular statements

If a method body, `if`, `for`, `foreach`, or `while` controls exactly one simple statement, don't wrap it in a block. This isn't a stylistic nicety here — it's already the pattern the implementation plans use (`if (_currentRequestId is null) return;`, `if (profile is null) return;`) and it should be applied consistently rather than only where it happened to get written that way first.
...
{
    _debugLogPanel.AppendLine("Ignored outcome with no pending request.");
    return;
}
```
